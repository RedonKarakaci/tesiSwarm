...
def evaluate(test_loader, config, model_path):
    ...
    if use_wandb:
        ...
        wandb.init(project="StarGate", group="evaluation")
    ...
    # Create gate classifier model
    gate_classifier_model = GateClassifier(num_channels_start=num_channels_start, dropout_p=dropout_p)
    gate_classifier_model.load_state_dict(torch.load(model_path)['gate_classifier_state_dict'])
    gate_classifier_model.to(device)
    ...
    for epoch in range(epochs):
        ...
        # Validation
        num_val_iterations = len(test_loader)
        gate_classifier_model.eval()
        with torch.no_grad():
        ...
        print('\n')
        print('Avg BCE Loss Val: ', val_bce)
        print('Avg Accuracy Loss Val: ', val_accuracy)
        print('Avg F1 Loss Val: ', val_f1)
        print('Avg AUROC Loss Val: ', val_auroc)
        print('-------------------------------------------------------------')

        if use_wandb:
            wandb.watch(gate_classifier_model, loss_bce, log="all", log_freq=500)
            wandb.log({'epoch': epoch, 'val_bce': val_bce,
